...
class UNet(torch.nn.Module):
    def __init__(self, channels, time_embedding_size=100):
        ...
        self.label_embedding = torch.nn.Embedding(10, time_embedding_size)
    ...
    def forward(self, xt, t, y=None):
        ...
        if y is not None:
            positional_embedding += self.label_embedding(y)
        ...
        for epoch in range(10):
            ...
            for x, y in tqdm(dataloader):
                ...
                if torch.rand(1).item() < 0.1:
                    y = None
                hat_eps, hat_mu = diffusion(xt, t, y)
    ...
    if sample == True:
        ...
        with torch.no_grad():
            for t in tqdm(range(diffusion.T)):
                ...
                cond_eps = -cond_score * torch.sqrt(1-diffusion.bar_alpha_schedule[t-1])
                frac_term1 = (1-diffusion.alpha_schedule[t-1])/(torch.sqrt(1-diffusion.bar_alpha_schedule[t-1]))
                frac_term2 = 1/torch.sqrt(diffusion.alpha_schedule[t-1])
